...
static inline uint64_t hash_len_17_to_32(const char *s, size_t len)
{
  ...
  return hash_len_16(rotate64(a + b, 43) + rotate64(c, 30) + d, a + rotate64(b + k2, 18) + c, mul);
}
...
static inline uint64_t hash_len_33_to_64(const char *s, size_t len)
{
  ...
  return hash_len_16(rotate64(e + f, 43) + rotate64(g, 30) + h, e + rotate64(f + a, 18) + g, mul);
}
...
static inline pair64 weak_hash_len_32_with_seeds2(uint64_t w, uint64_t x, uint64_t y, uint64_t z, uint64_t a,
                                                  uint64_t b)
{
  uint64_t c;
  pair64 result;
...
{
  return weak_hash_len_32_with_seeds2(fetch64(s), fetch64(s + 8), fetch64(s + 16), fetch64(s + 24), a, b);
}
...
uint64_t hash_data(const data_t data)
{
  return cfarmhash(data_base(data), data_size(data));
}
