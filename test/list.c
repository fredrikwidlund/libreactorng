...
void test_core(__attribute__((unused)) void **state)
{
  ...
  i = 0;
  list_foreach(&l, s) assert_string_equal(*s, a[i++]);

  i = sizeof a / sizeof *a;
  list_foreach_reverse(&l, s) assert_string_equal(*s, a[--i]);
...
int main()
{
  const struct CMUnitTest tests[] = {cmocka_unit_test(test_list_release), cmocka_unit_test(test_core),
                                     cmocka_unit_test(test_unit), cmocka_unit_test(test_edge)};
